        ...
        <nav class="flex items-center gap-6">
            @if(Auth::check() && Auth::user()->is_admin)
                <div class="hidden lg:flex items-center gap-5 border-r border-slate-200 pr-5">
                    ...
                    <a href="/admin/pos" class="text-sm font-medium {{ Request::is('admin/pos*') ? 'text-pink-600 font-semibold' : 'text-slate-600 hover:text-pink-600' }} transition">
                        POS System
                    </a>
                    <a href="/admin/report" class="text-sm font-medium {{ Request::is('admin/report*') ? 'text-pink-600 font-semibold' : 'text-slate-600 hover:text-pink-600' }} transition">
                        Reports
                    </a>
                </div>
            @else
                <a href="/home" class="text-sm font-medium text-slate-700 hover:text-pink-600 transition">
                    Home
                </a>
                <a href="/services" class="text-sm font-medium text-slate-700 hover:text-pink-600 transition">
                    Services
                </a>
                <a href="/gallery" class="text-sm font-medium text-slate-700 hover:text-pink-600 transition">
                    Gallery
                </a>
                <a href="/about" class="text-sm font-medium text-slate-700 hover:text-pink-600 transition">
                    About
                </a>
                <a href="/contact" class="text-sm font-medium text-slate-700 hover:text-pink-600 transition">
                    Contact
                </a>
                @auth
                    <a href="/appointments" class="text-sm font-medium text-slate-700 hover:text-pink-600 transition">
                        Booking
                    </a>
                @endauth
            @endif
            ...
            @auth
                <div class="relative">
                    <button onclick="toggleProfileMenu()" class="flex items-center gap-3 rounded-full border border-pink-100 bg-white px-3 py-2 shadow-sm hover:shadow-lg transition-all">
            ...
            @else
                <a href="/login" class="rounded-full border border-pink-200 px-5 py-2 text-sm font-semibold text-pink-600 hover:bg-pink-50 transition">
                    Login
                </a>
                <a href="/register" class="rounded-full bg-gradient-to-r from-pink-500 to-rose-500 px-5 py-2 text-sm font-semibold text-white shadow-lg shadow-pink-200 hover:scale-[1.03] transition">
                    Register
                </a>
            @endauth
        </nav>
